...
class Graph:
    ...
    @classmethod
    def from_dataframe(cls, interactome_dataframe, params=dict()): 
        assert interactome_dataframe.columns[0] == 'protein1'
        assert interactome_dataframe.columns[1] == 'protein2'
        assert 'cost' in interactome_dataframe.columns
        interactome_dataframe.to_csv('./_tmp_interactome.txt', sep='\t', index=False, header=True)
        return cls('./_tmp_interactome.txt', params)
    ...
    def _prepare_prizes(self, prizes_dataframe):
        ...
        duplicated_genes = prizes_dataframe[prizes_dataframe.set_index('name').index.duplicated()]['name'].tolist()
        if len(duplicated_genes) > 0:
            logger.info("Duplicated gene symbols in the prize file (we'll keep the max prize):")
            logger.info(duplicated_genes)
        ...
        missing_prizes = prizes_dataframe[prizes_dataframe.index == -1]['name'].tolist()
        if len(missing_prizes) > 0:
            logger.info("Members of the prize file not present in the interactome:")
            logger.info(missing_prizes)
    ...
    def pcsf(self, pruning="strong", verbosity_level=0):
        ...
        return np.array(vertex_indices, dtype=int), np.array(edge_indices, dtype=int)
    ...
    def output_forest_as_networkx(self, vertex_indices, edge_indices):
        ...
        try: 
            ...
            logger.info(vertex_indices)
            logger.info(list(self.nodes))
    ...
    def randomizations(self, noisy_edges_reps=0, random_terminals_reps=0):
        ...
        vertex_indices.index = np.array(vertex_indices.index, dtype=int)
        edge_indices = pd.concat([robust_edges, specific_edges], axis=1).fillna(0)
        edge_indices.index = np.array(edge_indices.index, dtype=int)
        ...
        nx.set_edge_attributes(forest, self.interactome_dataframe[["protein1", "protein2"]].join(edge_indices, how="right").set_index(["protein1", "protein2"]).to_dict(orient="index"))
...
def louvain_clustering(nxgraph):
    ...
    Arguments:
        nxgraph (networkx.Graph): a networkx graph, usually the augmented_forest.
    """
    nx.set_node_attributes(nxgraph, {node: {'louvain_clusters':str(cluster)} for node,cluster in community_louvain.best_partition(nxgraph).items()})
...
def filter_graph_by_component_size(nxgraph, min_size=5):
    ...
    small_components = [g for g in nx.connected_components(nxgraph) if len(g) < min_size]
