...
class FeatureData: 
	...
	def initialize_qtl(self, qtl_dir, qtl_fdr): 
		...
		self._qtl_accessor = TensorQTLManager.load_config(self._qtl_config_path)
	...
	@property
	def regions(self):
		if self._regions is None: self._regions, self._lengths = load_counts_metadata(self._counts_data_paths["metadata"])
		return self._regions

	@property
	def lengths(self):
		if self._lengths is None: self._regions, self._lengths = load_counts_metadata(self._counts_data_paths["metadata"])
	...
	def residualizer(self):
		if self._residualizer is None: self._residualizer = Residualizer.load_covariates(path=self._covariates_path)
