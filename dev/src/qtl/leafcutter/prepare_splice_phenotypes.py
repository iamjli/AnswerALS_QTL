...
def stream_table(f, ss = ''):
    ...
    for ln in f:  # iterate through rows
        ln = ln.strip().split(ss)
        attr = {}  # counts keyed by header

        for i in range(len(head)):  # iterate by column
...
def main(ratio_file, chroms, blacklist_chroms, pcs=50):
    ...
    for dic in stream_table(gzip.open(ratio_file),' '):  # yields a dic of counts keyed by header value for each row 

        # parse and extract chromosome
        chrom = dic['chrom'].replace("chr",'')
        chr_ = chrom.split(":")[0]
        if chr_ in blacklist_chroms: continue  # skip blacklisted i.e. X/Y,M

        NA_indices, valRow, aveReads = [], [], []
        tmpvalRow = []

        # populate several lists for each sample
        # `NA_indices` - site has no counts
        # `valRow` - fraction of reads in cluster that map to that site
        # `aveReads` - fraction of reads in cluster that map to that site
        i = 0
        for sample in header:
            ...
            if float(denom) < 1:  # dont count any sites where total in cluster is 0
                count = "NA"
                tmpvalRow.append("NA")
                NA_indices.append(i)
            else:
        ...
        ave = np.mean(aveReads)  # average ratio across samples 
